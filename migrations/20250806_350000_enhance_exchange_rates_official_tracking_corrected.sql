-- Enhancement: Official Exchange Rates Tracking (Corrected)
...
CREATE INDEX IF NOT EXISTS idx_exchange_rates_official ON exchange_rates(is_official, rate_date, source) WHERE is_official = true;
...
SET 
    rate_source_type = CASE 
        WHEN source ILIKE '%manual%' OR source IS NULL THEN 'MANUAL'
        WHEN source ILIKE '%api%' THEN 'API'
        ELSE 'MANUAL'
    END,
    is_official = FALSE,
...
-- Update constraint to include OFFICIAL rate type
ALTER TABLE exchange_rates 
DROP CONSTRAINT IF EXISTS exchange_rates_rate_type_check;

ALTER TABLE exchange_rates 
ADD CONSTRAINT exchange_rates_rate_type_check 
CHECK (rate_type IN ('SPOT', 'CLOSING', 'AVERAGE', 'HISTORICAL', 'BUDGET', 'OFFICIAL'));
...
ORDER BY rate_date DESC, source, from_currency;
...
-- Create audit trigger for official rates changes (if audit_log exists)
...
FROM exchange_rates
WHERE is_official = true
GROUP BY source, rate_source_type
...
-- Create official rate sources registry
...
-- Create trigger to update updated_at timestamp for official_rate_sources
...
BEGIN
    ...
    RAISE NOTICE 'New columns added: rate_source_type, is_official, publication_date, updated_by';
END $$;