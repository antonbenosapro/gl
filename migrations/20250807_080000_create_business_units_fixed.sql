-- =====================================================
-- Create Unified Business Units Table (FIXED)
-- =====================================================
-- Author: Claude Code Assistant
-- Date: August 7, 2025
-- Description: Create unified business_units table - fixed version
--              addressing foreign key constraints and references
-- =====================================================

-- Step 1: Create the unified business_units table (without problematic foreign keys initially)
CREATE TABLE business_units (
    ...
    company_code_id VARCHAR(10) NOT NULL DEFAULT 'C001',
...
-- Step 2: Add foreign key constraints (that exist)
-- Only add FKs for tables that actually exist
DO $$
BEGIN
    -- Check and add business_areas FK if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'business_areas') THEN
        ALTER TABLE business_units 
        ADD CONSTRAINT fk_business_units_business_area 
        FOREIGN KEY (business_area_id) REFERENCES business_areas(business_area_id);
    END IF;
    
    -- Check and add reporting_locations FK if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reporting_locations') THEN
        ALTER TABLE business_units 
        ADD CONSTRAINT fk_business_units_location 
        FOREIGN KEY (location_code) REFERENCES reporting_locations(location_code);
    END IF;
    
    -- Check and add product_lines FK if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'product_lines') THEN
        ALTER TABLE business_units 
        ADD CONSTRAINT fk_business_units_product_line 
        FOREIGN KEY (product_line_id) REFERENCES product_lines(product_line_id);
    END IF;
END $$;

-- Step 3: Add self-referencing foreign key after table is created
ALTER TABLE business_units 
...
-- Step 5: Create trigger for automatic modified timestamp update
...
-- Step 6: Create helper functions for business unit operations
...
-- Step 7: Create enhanced views for hierarchical and analytical reporting
...
-- 
-- Status: Table structure ready for data migration
-- =====================================================